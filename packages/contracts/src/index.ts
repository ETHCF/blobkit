...
export function getContractAddress(chainId: number, contract: keyof ContractAddresses): string {
  ...
  if (!address) {
    throw new Error(`${contract} contract not deployed on chain ${chainId}. Please set the appropriate environment variable.`);
  }
...
export const createJobCreatedFilter = (jobId?: string, user?: string) => {
  const topics = ["0x" + "JobCreated".padEnd(64, "0")]; // Placeholder
  if (jobId) topics.push(jobId);
  if (user) topics.push(user);
  return { topics };
};

export const createJobCompletedFilter = (jobId?: string) => {
  const topics = ["0x" + "JobCompleted".padEnd(64, "0")]; // Placeholder
  if (jobId) topics.push(jobId);
  return { topics };
};
