const abi = [
  {
    type: 'constructor',
    inputs: [{ name: '_owner', type: 'address', internalType: 'address' }],
    stateMutability: 'nonpayable'
  },
  {
    type: 'function',
    name: 'DEFAULT_JOB_TIMEOUT',
    inputs: [],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'MAX_PROXY_FEE_PERCENT',
    inputs: [],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'authorizedProxies',
    inputs: [{ name: '', type: 'address', internalType: 'address' }],
    outputs: [{ name: '', type: 'bool', internalType: 'bool' }],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'completeJob',
    inputs: [
      { name: 'jobId', type: 'bytes32', internalType: 'bytes32' },
      { name: 'blobTxHash', type: 'bytes32', internalType: 'bytes32' },
      { name: 'proof', type: 'bytes', internalType: 'bytes' }
    ],
    outputs: [],
    stateMutability: 'nonpayable'
  },
  {
    type: 'function',
    name: 'depositForBlob',
    inputs: [{ name: 'jobId', type: 'bytes32', internalType: 'bytes32' }],
    outputs: [],
    stateMutability: 'payable'
  },
  {
    type: 'function',
    name: 'emergencyWithdraw',
    inputs: [],
    outputs: [],
    stateMutability: 'nonpayable'
  },
  {
    type: 'function',
    name: 'getJob',
    inputs: [{ name: 'jobId', type: 'bytes32', internalType: 'bytes32' }],
    outputs: [
      {
        name: 'job',
        type: 'tuple',
        internalType: 'struct BlobKitEscrow.Job',
        components: [
          { name: 'user', type: 'address', internalType: 'address' },
          { name: 'amount', type: 'uint256', internalType: 'uint256' },
          { name: 'completed', type: 'bool', internalType: 'bool' },
          { name: 'timestamp', type: 'uint256', internalType: 'uint256' },
          { name: 'blobTxHash', type: 'bytes32', internalType: 'bytes32' }
        ]
      }
    ],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'getJobTimeout',
    inputs: [],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'getProxyFee',
    inputs: [{ name: 'proxy', type: 'address', internalType: 'address' }],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'isJobExpired',
    inputs: [{ name: 'jobId', type: 'bytes32', internalType: 'bytes32' }],
    outputs: [{ name: '', type: 'bool', internalType: 'bool' }],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'jobTimeout',
    inputs: [],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'jobs',
    inputs: [{ name: '', type: 'bytes32', internalType: 'bytes32' }],
    outputs: [
      { name: 'user', type: 'address', internalType: 'address' },
      { name: 'amount', type: 'uint256', internalType: 'uint256' },
      { name: 'completed', type: 'bool', internalType: 'bool' },
      { name: 'timestamp', type: 'uint256', internalType: 'uint256' },
      { name: 'blobTxHash', type: 'bytes32', internalType: 'bytes32' }
    ],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'owner',
    inputs: [],
    outputs: [{ name: '', type: 'address', internalType: 'address' }],
    stateMutability: 'view'
  },
  { type: 'function', name: 'pause', inputs: [], outputs: [], stateMutability: 'nonpayable' },
  {
    type: 'function',
    name: 'paused',
    inputs: [],
    outputs: [{ name: '', type: 'bool', internalType: 'bool' }],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'proxyFees',
    inputs: [{ name: '', type: 'address', internalType: 'address' }],
    outputs: [{ name: '', type: 'uint256', internalType: 'uint256' }],
    stateMutability: 'view'
  },
  {
    type: 'function',
    name: 'refundExpiredJob',
    inputs: [{ name: 'jobId', type: 'bytes32', internalType: 'bytes32' }],
    outputs: [],
    stateMutability: 'nonpayable'
  },
  {
    type: 'function',
    name: 'renounceOwnership',
    inputs: [],
    outputs: [],
    stateMutability: 'nonpayable'
  },
  {
    type: 'function',
    name: 'setJobTimeout',
    inputs: [{ name: '_timeout', type: 'uint256', internalType: 'uint256' }],
    outputs: [],
    stateMutability: 'nonpayable'
  },
  {
    type: 'function',
    name: 'setProxyAuthorization',
    inputs: [
      { name: 'proxy', type: 'address', internalType: 'address' },
      { name: 'authorized', type: 'bool', internalType: 'bool' }
    ],
    outputs: [],
    stateMutability: 'nonpayable'
  },
  {
    type: 'function',
    name: 'setProxyFee',
    inputs: [{ name: 'percent', type: 'uint256', internalType: 'uint256' }],
    outputs: [],
    stateMutability: 'nonpayable'
  },
  {
    type: 'function',
    name: 'transferOwnership',
    inputs: [{ name: 'newOwner', type: 'address', internalType: 'address' }],
    outputs: [],
    stateMutability: 'nonpayable'
  },
  { type: 'function', name: 'unpause', inputs: [], outputs: [], stateMutability: 'nonpayable' },
  {
    type: 'event',
    name: 'JobCompleted',
    inputs: [
      { name: 'jobId', type: 'bytes32', indexed: true, internalType: 'bytes32' },
      { name: 'blobTxHash', type: 'bytes32', indexed: false, internalType: 'bytes32' },
      { name: 'proxyFee', type: 'uint256', indexed: false, internalType: 'uint256' }
    ],
    anonymous: false
  },
  {
    type: 'event',
    name: 'JobCreated',
    inputs: [
      { name: 'jobId', type: 'bytes32', indexed: true, internalType: 'bytes32' },
      { name: 'user', type: 'address', indexed: true, internalType: 'address' },
      { name: 'amount', type: 'uint256', indexed: false, internalType: 'uint256' }
    ],
    anonymous: false
  },
  {
    type: 'event',
    name: 'JobRefunded',
    inputs: [
      { name: 'jobId', type: 'bytes32', indexed: true, internalType: 'bytes32' },
      { name: 'reason', type: 'string', indexed: false, internalType: 'string' }
    ],
    anonymous: false
  },
  {
    type: 'event',
    name: 'JobTimeoutUpdated',
    inputs: [
      { name: 'oldTimeout', type: 'uint256', indexed: false, internalType: 'uint256' },
      { name: 'newTimeout', type: 'uint256', indexed: false, internalType: 'uint256' }
    ],
    anonymous: false
  },
  {
    type: 'event',
    name: 'OwnershipTransferred',
    inputs: [
      { name: 'previousOwner', type: 'address', indexed: true, internalType: 'address' },
      { name: 'newOwner', type: 'address', indexed: true, internalType: 'address' }
    ],
    anonymous: false
  },
  {
    type: 'event',
    name: 'Paused',
    inputs: [{ name: 'account', type: 'address', indexed: false, internalType: 'address' }],
    anonymous: false
  },
  {
    type: 'event',
    name: 'ProxyAuthorizationChanged',
    inputs: [
      { name: 'proxy', type: 'address', indexed: true, internalType: 'address' },
      { name: 'authorized', type: 'bool', indexed: false, internalType: 'bool' }
    ],
    anonymous: false
  },
  {
    type: 'event',
    name: 'ProxyFeeUpdated',
    inputs: [
      { name: 'proxy', type: 'address', indexed: true, internalType: 'address' },
      { name: 'oldFee', type: 'uint256', indexed: false, internalType: 'uint256' },
      { name: 'newFee', type: 'uint256', indexed: false, internalType: 'uint256' }
    ],
    anonymous: false
  },
  {
    type: 'event',
    name: 'Unpaused',
    inputs: [{ name: 'account', type: 'address', indexed: false, internalType: 'address' }],
    anonymous: false
  },
  { type: 'error', name: 'InvalidJobTimeout', inputs: [] },
  { type: 'error', name: 'InvalidProof', inputs: [] },
  { type: 'error', name: 'InvalidProxyFee', inputs: [] },
  { type: 'error', name: 'JobAlreadyCompleted', inputs: [] },
  { type: 'error', name: 'JobAlreadyExists', inputs: [] },
  { type: 'error', name: 'JobNotExpired', inputs: [] },
  { type: 'error', name: 'JobNotFound', inputs: [] },
  { type: 'error', name: 'TransferFailed', inputs: [] },
  { type: 'error', name: 'UnauthorizedProxy', inputs: [] },
  { type: 'error', name: 'ZeroAmount', inputs: [] }
];

export default abi;
