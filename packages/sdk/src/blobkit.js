...
import { generateJobId, calculatePayloadHash, discoverProxyUrl, getDefaultEscrowContract, validateBlobSize, formatEther, parseEther, isValidAddress, validateEnvironmentConfig } from './utils.js';
import { initializeKzg, encodeBlob, blobToKzgCommitment, computeKzgProof, commitmentToVersionedHash, bytesToHex } from './kzg.js';
/**
...
export class BlobKit {
    ...
    async estimateCost(payload) {
        ...
        try {
            ...
            const blobGasPrice = ethersFeeData.maxFeePerBlobGas ?? ethers.parseUnits('1', 'gwei');
            ...
            throw new BlobKitError(BlobKitErrorCode.NETWORK_ERROR, 'Failed to estimate cost', error instanceof Error ? error : undefined);
        }
    }
    /**
    ...
    async refundIfExpired(jobId) {
        try {
            ...
            throw new BlobKitError(BlobKitErrorCode.NETWORK_ERROR, 'Failed to refund expired job', error instanceof Error ? error : undefined);
        }
    }
    /**
     ...
     */
    generateJobId(userAddress, payloadHash, nonce) {
     ...
     */
    async writeWithWeb3Payment(payload, meta) {
        if (!this.signer) {
     ...
     */
    async writeWithDirectTransaction(payload, meta) {
        if (!this.signer) {
     ...
     */
    async submitToProxy(proxyUrl, data) {
        ...
        if (!response.ok) {
            ...
            throw new BlobKitError(BlobKitErrorCode.PROXY_ERROR, `Proxy request failed: ${response.status} ${errorText}`);
        }
     ...
     */
    async sendBlobTransaction(blob, commitment, proof) {
        if (!this.signer) {
...
//# sourceMappingURL=blobkit.js.map