...
export async function initializeKzg() {
    ...
    try {
        ...
        if (!trustedSetupPath) {
            ...
            throw new BlobKitError(BlobKitErrorCode.KZG_ERROR, 'BLOBKIT_KZG_TRUSTED_SETUP_PATH environment variable must be set. ' +
        ...
        try {
            ...
            throw new BlobKitError(BlobKitErrorCode.KZG_ERROR, `Trusted setup file not found at path: ${trustedSetupPath}`);
        }
        ...
        throw new BlobKitError(BlobKitErrorCode.KZG_ERROR, 'Failed to initialize KZG trusted setup', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export function encodeBlob(data) {
    ...
    if (data.length > FIELD_ELEMENTS_PER_BLOB * BYTES_PER_FIELD_ELEMENT) {
        throw new BlobKitError(BlobKitErrorCode.BLOB_TOO_LARGE, `Data too large: ${data.length} bytes, max: ${FIELD_ELEMENTS_PER_BLOB * BYTES_PER_FIELD_ELEMENT} bytes`);
    }
...
export function blobToKzgCommitment(blob) {
    ...
    if (blob.length !== BLOB_SIZE) {
        throw new BlobKitError(BlobKitErrorCode.INVALID_PAYLOAD, `Invalid blob size: expected ${BLOB_SIZE}, got ${blob.length}`);
    }
    try {
        return kzg.blobToKzgCommitment(blob);
    }
    catch (error) {
        throw new BlobKitError(BlobKitErrorCode.KZG_ERROR, 'Failed to compute KZG commitment', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export function computeKzgProof(blob, commitment) {
    if (!isSetupLoaded) {
        throw new BlobKitError(BlobKitErrorCode.KZG_ERROR, 'KZG not initialized. Call initializeKzg() first.');
    }
    if (blob.length !== BLOB_SIZE) {
    ...
    try {
        ...
        throw new BlobKitError(BlobKitErrorCode.KZG_ERROR, 'Failed to compute KZG proof', error instanceof Error ? error : undefined);
    }
}
/**
 * Verify KZG proof
 ...
 */
export function verifyKzgProof(blob, commitment, proof) {
    if (!isSetupLoaded) {
        throw new BlobKitError(BlobKitErrorCode.KZG_ERROR, 'KZG not initialized. Call initializeKzg() first.');
    }
    try {
        ...
        throw new BlobKitError(BlobKitErrorCode.KZG_ERROR, 'Failed to verify KZG proof', error instanceof Error ? error : undefined);
    }
}
/**
 ...
 */
export async function commitmentToVersionedHash(commitment) {
    if (commitment.length !== 48) {
        throw new BlobKitError(BlobKitErrorCode.INVALID_PAYLOAD, `Invalid commitment size: expected 48 bytes, got ${commitment.length}`);
    }
